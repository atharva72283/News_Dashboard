...
AI-powered sentiment analysis via Claude API.
...
# ─────────────────────────────────────────────
# AI SENTIMENT via Claude API
# ─────────────────────────────────────────────

@st.cache_data(ttl=3600, show_spinner=False)
def get_sentiment_ai(titles_tuple: tuple) -> dict:
    """
    Uses Claude AI to analyze sentiment for a batch of news headlines.
    Returns a dict: { headline -> 'positive' | 'negative' | 'neutral' }

    Batching is used to minimize API calls — up to 50 headlines per call.
    Results are cached for 1 hour to avoid re-analyzing the same headlines.
    """
    ...
    if not api_key:
        # Fallback to basic heuristic if no API key configured
...
Rules:
- "positive" = good for markets, investors, or the subject company (e.g. FII buying, strong earnings, rate cuts, policy support, upgrades, capacity addition)
- "negative" = bad for markets, investors, or the subject company (e.g. FII selling, fraud, default, downgrade, crash, ban, penalty, geopolitical risk)
- "neutral" = factual/informational with no clear market impact (e.g. RBI circular, policy announcement without clear direction, routine filing)

IMPORTANT context rules:
- "FII buying" or "DII buying" = POSITIVE (institutional accumulation is bullish)
...
- A headline can only be one of: positive, negative, neutral
...
def _fallback_sentiment(title: str) -> str:
    """
    Very minimal fallback used ONLY if Claude API is unavailable.
    Much simpler than before — just catches obvious cases.
    """
    t = title.lower()
