...
def ai_sentiment(title: str) -> str:
    """
    Ask Mistral to classify a headline as positive/negative/neutral.
    Returns one of: 'positive', 'negative', 'neutral'.
    Falls back to keyword-based get_sentiment() if API fails.
    TTL=3600 so we don't hammer the API on every rerun.
    """
    prompt = (
...
def ai_search_news(query: str) -> list:
    """
    1. Google News RSS search with the raw user query (any form: name/ISIN/NSE/BSE code).
    2. Collect up to 20 raw headlines.
    3. Ask Mistral to pick the top 5 most relevant + give sentiment.
    Returns list of dicts with title, link, dt, sentiment, ai_analysed=True.
    """
    # Step 1 — fetch Google News RSS for the query directly
    seen, raw_items = set(), []
    ...
    # Step 2 — Ask Mistral to rank and pick top 5 + classify sentiment
    headlines_block = "\n".join(
        f"{i+1}. {a['title']}" for i, a in enumerate(raw_items[:20])
    )
    prompt = (
    ...
    # fallback — return top 5 with keyword sentiment if AI gave nothing
    if not selected:
...
def ai_enrich_articles(articles: list, batch_size: int = 10) -> list:
    """
    Enrich a list of articles with AI sentiment.
    Works in batches to minimise API calls.
    Articles that already have ai_analysed=True are skipped.
    Returns a new list with updated sentiment + ai_analysed flag.
    """
    out = []
    ...
    for i in range(0, len(needs_ai), batch_size):
        ...
        prompt = (
            ...
            "Context: India stock market, RBI, SEBI, rupee, economy, geopolitics.\n\n"
            f"Headlines:\n{headlines_block}\n\n"
            "Respond in this EXACT format (one per line):\n"
            "NUMBER|SENTIMENT\n"
            "Example: 1|negative"
        )
        result = _mistral_call([{"role": "user", "content": prompt}],
...
def get_yesterday_range():
    """Return (start_dt, end_dt) for previous calendar day in UTC."""
    now_ist  = datetime.now(IST)
    yday_ist = now_ist - timedelta(days=1)
    start = IST.localize(datetime(yday_ist.year, yday_ist.month, yday_ist.day, 0, 0, 0)).astimezone(timezone.utc)
    end   = IST.localize(datetime(yday_ist.year, yday_ist.month, yday_ist.day, 23, 59, 59)).astimezone(timezone.utc)
    return start, end

def get_top10_yesterday(all_articles: list) -> list:
    """
    Filter all_articles for previous day and return top 10 by:
    1. Priority first
    2. Then negative sentiment
    3. Then chronological (newest first)
    AI sentiment is applied if available.
    """
    start, end = get_yesterday_range()
    yday = []
    for a in all_articles:
        dt = a.get("dt", datetime.now(timezone.utc))
        ...
        if start <= dt <= end:
            yday.append({**a, "dt": dt})
    # Sort: priority desc, then negative first, then newest
    def sort_key(a):
        p = 0 if a.get("priority") else 1
        s = 0 if a.get("sentiment") == "negative" else (1 if a.get("sentiment") == "positive" else 2)
        return (p, s, -a["dt"].timestamp())
    yday.sort(key=sort_key)
    # Deduplicate
    seen, out = set(), []
    for a in yday:
...
def get_portfolio_monthly_news(portfolio: list) -> dict:
    """
    For top 20 portfolio stocks by exposure, fetch last 30 days news.
    Returns dict: {stock_name: [articles]}
    """
    ...
    for stock in top20:
        ...
        name = stock.get("name", "")
        if not nse and not name:
            continue
        arts = fetch_stock_news_gn(nse, name)   # already has 30-day window
        if arts:
            result[name] = arts
    return result
...
def fetch_fii_flow():
    """
    Fetches FII/DII provisional cash market data.
    3-tier fallback: nselib → NSE JSON API → Google News
    """
    result = {"fii_net":None,"dii_net":None,"headline":None,
              "dt":None,"data_date":None,"source":None}

    def _make_result(fii_v, dii_v, date_v, source_label):
        fp = "+" if fii_v >= 0 else ""
        dp = "+" if (dii_v or 0) >= 0 else ""
        return {
            "fii_net":fii_v, "dii_net":dii_v,
            "data_date":str(date_v)[:12],
            "dt":datetime.now(timezone.utc),
            "source":source_label,
            "headline": (f"FII: {fp}{fii_v:,.2f} Cr  |  "
                         f"DII: {dp}{(dii_v or 0):,.2f} Cr  (as of {str(date_v)[:12]})")
        }
    ...
    if NSELIB_OK:
        try:
            from nselib import capital_market
            # nselib wraps NSE FII/DII API — try the correct function
            df = capital_market.fii_dii_trade_react()
            if df is not None and not df.empty:
                # Print columns for debugging (appears in Streamlit Cloud logs)
                row = df.iloc[0]
                cols_lower = {c.lower(): c for c in df.columns}
                # Find date
                date_v = ""
                for dk in ["date","trade_date","tradedate","dt"]:
                    if dk in cols_lower:
                        date_v = str(row[cols_lower[dk]])[:12]; break
                # Find FII — scan all columns for anything containing fii
                fii_v, dii_v = None, None
                for col in df.columns:
                    cl = col.lower()
                    if "fii" in cl and ("net" in cl or "trade" in cl or cl == "fii"):
                        v = _to_float(row[col])
                        if v is not None: fii_v = v; break
                for col in df.columns:
                    cl = col.lower()
                    if "dii" in cl and ("net" in cl or "trade" in cl or cl == "dii"):
                        v = _to_float(row[col])
                        if v is not None: dii_v = v; break
                if fii_v is not None:
                    return _make_result(fii_v, dii_v, date_v, "NSE (nselib)")
        except: pass

    # ── TIER 2: Direct NSE JSON API ────────────────────────────
    try:
        ...
        sess.get("https://www.nseindia.com",
                 headers={**hdrs, "Accept":"text/html,application/xhtml+xml,*/*;q=0.9"},
        ...
        resp = sess.get(
            ...
            timeout=15, verify=False, proxies=PROXIES or {}
        )
        if resp.status_code == 200:
            data = resp.json()
            if isinstance(data, list) and len(data) > 0:
                row = data[0]
                # NSE API returns nested structure — flatten and scan all keys
                flat = {}
                def _flatten(obj, prefix=""):
                    if isinstance(obj, dict):
                        for k,v in obj.items():
                            _flatten(v, f"{prefix}{k}_" if prefix else k)
                    else:
                        flat[prefix.rstrip("_")] = obj
                _flatten(row)
                # Extract date
                date_v = ""
                for dk in ["date","Date","TRADE_DATE","tradeDate"]:
                    if dk in flat and flat[dk]:
                        date_v = str(flat[dk])[:12]; break
                # Extract FII and DII — scan flattened keys
                fii_v, dii_v = None, None
                for k,v in flat.items():
                    kl = k.lower()
                    if "fii" in kl and ("net" in kl or "trade" in kl) and fii_v is None:
                        fii_v = _to_float(v)
                    if "dii" in kl and ("net" in kl or "trade" in kl) and dii_v is None:
                        dii_v = _to_float(v)
                if fii_v is not None:
                    return _make_result(fii_v, dii_v, date_v, "NSE API")
    except: pass

    # ── TIER 3: Google News ─────────────────────────────────────
    try:
        for q in ["FII DII provisional net NSE India crore",
                  "FII net buy sell India NSE crore today"]:
            ...
            for e in feed.entries[:10]:
                ...
                if ("fii" in tl or "fpi" in tl) and "dii" in tl and is_recent(dt, days=1):
                    ...
                    valid = [float(n.replace(",","")) for n in nums[:6]
                             if _to_float(n.replace(",","")) is not None]
                    result.update({
...
with st.sidebar:
    ...
    if search_input.strip():
        ...
        if res["found"]:
            ...
            # Use AI-powered Google search news for any query form
            ...
            if ai_news:
                for art in ai_news[:5]:
                    ...
                    ai_badge = '<span style="background:#e8eaf6;color:#3949ab;font-size:7pt;padding:1px 4px;border-radius:3px;float:right;">🤖 AI Analysed</span>' if ai_flag else ""
                ...
                # fallback to standard GN search
            ...
            # Not found as ticker — try pure AI Google news search
...
# AI sentiment badge shown in lower-right of every card.
# ─────────────────────────────────────────────────────────────
...
def _ai_badge(ai_analysed: bool) -> str:
    if not ai_analysed:
        return ""
    return '<span style="background:#e8eaf6;color:#3949ab;font-size:7pt;font-weight:600;padding:1px 5px;border-radius:3px;margin-left:auto;">🤖 AI Analysed</span>'
...
def render_morning_briefing(all_articles, portfolio):
    """
    Morning Briefing tab:
    1. Top 10 news from yesterday in a table with green/red sentiment highlights.
    2. Last month news for top 20 portfolio exposures.
    """
    yday_ist = (datetime.now(IST) - timedelta(days=1)).strftime("%d %b %Y")
    st.markdown(f"<div style='font-size:12pt;font-weight:700;color:#1a237e;margin-bottom:6px;'>📋 Top 10 News — {yday_ist}</div>", unsafe_allow_html=True)
    st.caption("AI-analysed · Priority and negative items ranked first · Click headline to open")

    top10 = get_top10_yesterday(all_articles)

    if not top10:
        st.info(f"No news articles found for {yday_ist}. This may be because the dashboard just started or it is the first day of data collection.")
    else:
        # Render as styled HTML table
        rows_html = ""
        for i, art in enumerate(top10, 1):
            sent = art.get("sentiment","neutral")
            ai_a = art.get("ai_analysed", False)
            ip   = art.get("priority", False)

            sent_bg  = "#e8f5e9" if sent=="positive" else "#ffebee" if sent=="negative" else "#f5f5f5"
            sent_col = "#2e7d32" if sent=="positive" else "#c62828" if sent=="negative" else "#546e7a"
            sent_lbl = "▲ POSITIVE" if sent=="positive" else "▼ NEGATIVE" if sent=="negative" else "— NEUTRAL"
            prio_star = "⚡ " if ip else ""
            ai_cell  = '<span style="font-size:7.5pt;background:#e8eaf6;color:#3949ab;padding:1px 4px;border-radius:3px;">🤖 AI</span>' if ai_a else ""
            dt_lbl   = fmt_ist(art["dt"])
            title_link = f'<a href="{art["link"]}" target="_blank" style="color:#1a237e;font-weight:600;text-decoration:none;">{prio_star}{art["title"]}</a>'
            cat_lbl  = art.get("category","")[:25]

            rows_html += f"""
            <tr style="background:{sent_bg};">
              <td style="padding:6px 8px;font-size:9pt;color:#546e7a;text-align:center;width:28px;">{i}</td>
              <td style="padding:6px 8px;font-size:10pt;line-height:1.4;">{title_link}</td>
              <td style="padding:6px 8px;font-size:8.5pt;color:#546e7a;white-space:nowrap;">{dt_lbl}</td>
              <td style="padding:6px 8px;font-size:8.5pt;color:#546e7a;">{cat_lbl}</td>
              <td style="padding:6px 8px;font-size:8.5pt;font-weight:700;color:{sent_col};white-space:nowrap;text-align:center;">{sent_lbl}</td>
              <td style="padding:6px 8px;text-align:center;">{ai_cell}</td>
            </tr>"""
        ...
        <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
          <thead>
            <tr style="background:#1a237e;color:#fff;">
              <th style="padding:8px;font-size:9pt;">#</th>
              <th style="padding:8px;font-size:9pt;text-align:left;">Headline</th>
              <th style="padding:8px;font-size:9pt;white-space:nowrap;">Time (IST)</th>
              <th style="padding:8px;font-size:9pt;text-align:left;">Category</th>
              <th style="padding:8px;font-size:9pt;">Sentiment</th>
              <th style="padding:8px;font-size:9pt;">AI</th>
            </tr>
          </thead>
    ...
    # ── Section 2: Last month news for top 20 portfolio exposures ──
    st.markdown("---")
    if not portfolio:
        st.info("No portfolio loaded. Ask admin to import via the sidebar.")
        return

    top20 = sorted(portfolio, key=lambda x: x.get("position_crs",0), reverse=True)[:20]
    total_exp = sum(p.get("position_crs",0) for p in portfolio)

    st.markdown(
        f"<div style='font-size:12pt;font-weight:700;color:#4527a0;margin-bottom:4px;'>"
        f"📊 Last 30 Days News — Top {len(top20)} Exposures</div>",
        unsafe_allow_html=True
    )
    st.caption("Sorted by portfolio exposure (highest first) · Google News · last 30 days")

    for stock in top20:
        name = stock.get("name",""); nse = stock.get("nse_code",""); pos = stock.get("position_crs",0)
        pct_exp = (pos / total_exp * 100) if total_exp > 0 else 0
        arts = fetch_stock_news_gn(nse, name)

        pos_n = sum(1 for a in arts if a.get("sentiment")=="positive")
        neg_n = sum(1 for a in arts if a.get("sentiment")=="negative")
        risk_col = "#c62828" if neg_n > pos_n else "#2e7d32" if pos_n >= neg_n and arts else "#546e7a"

        st.markdown(
            f'<div style="margin:12px 0 6px;padding:6px 12px;background:#f0f2f5;border-left:4px solid {risk_col};border-radius:4px;">'
            f'<span style="font-weight:700;color:#1a237e;font-size:10.5pt;">{name}</span>'
            f'<span style="color:#718096;font-size:9pt;margin-left:10px;">{nse}</span>'
            + (f'<span style="background:#4527a0;color:#fff;font-size:8pt;padding:1px 7px;border-radius:10px;margin-left:8px;">₹{pos:,.1f} Cr ({pct_exp:.1f}%)</span>' if pos>0 else "")
            + (f'<span style="color:#2e7d32;font-size:9pt;font-weight:700;margin-left:10px;">🟢 {pos_n} pos</span>' if pos_n else "")
            + (f'<span style="color:#c62828;font-size:9pt;font-weight:700;margin-left:8px;">🔴 {neg_n} neg</span>' if neg_n else "")
            + f'</div>',
            unsafe_allow_html=True
        )

        if not arts:
            st.markdown("<div style='font-size:10pt;color:#718096;padding:2px 0 8px 12px;'>No news in last 30 days.</div>", unsafe_allow_html=True)
            continue
        ...
        for art in arts[:5]:
            ...
            ai_a = art.get("ai_analysed",False)
            ip   = art.get("priority",False)
            sent_bg  = "#f6fff7" if sent=="positive" else "#fff6f6" if sent=="negative" else "#fff"
            border   = "#43a047" if sent=="positive" else "#e53935" if sent=="negative" else "#e2e8f0"
            sent_lbl = '<span style="background:#e8f5e9;color:#2e7d32;font-size:7.5pt;font-weight:700;padding:1px 5px;border-radius:3px;">▲ POS</span>' if sent=="positive" else \
                       '<span style="background:#ffebee;color:#c62828;font-size:7.5pt;font-weight:700;padding:1px 5px;border-radius:3px;">▼ NEG</span>' if sent=="negative" else ""
            ai_b = '<span style="background:#e8eaf6;color:#3949ab;font-size:7pt;padding:1px 4px;border-radius:3px;">🤖 AI Analysed</span>' if ai_a else ""
            prio = '⚡ ' if ip else ""
            dt_o = art["dt"]
            if isinstance(dt_o, str):
                try:    dt_o = datetime.fromisoformat(dt_o)
            ...
            st.markdown(
                f'<div style="background:{sent_bg};border:1px solid {border};border-left:3px solid {border};'
                f'border-radius:5px;padding:7px 10px;margin:3px 0 3px 12px;">'
                f'<a href="{art["link"]}" target="_blank" style="font-size:10pt;font-weight:600;color:#1a237e;text-decoration:none;">{prio}{art["title"]}</a>'
                f'<div style="font-size:8.5pt;color:#718096;margin-top:3px;display:flex;justify-content:space-between;align-items:center;">'
                f'<span>{time_ago(dt_o)} {sent_lbl}</span><span>{ai_b}</span>'
                f'</div></div>',
                unsafe_allow_html=True
            )
...
# ─────────────────────────────────────────────────────────────
# AI ENRICH all_articles (batch — runs once per cache cycle)
# ─────────────────────────────────────────────────────────────

@st.cache_data(ttl=300, show_spinner=False)
def get_ai_enriched_articles(articles_key: str, articles_json: str) -> str:
    """
    Cache-friendly wrapper: serialize/deserialize articles list for AI enrichment.
    articles_key is a hash of the article titles for cache invalidation.
    """
    import json as _json
    arts = _json.loads(articles_json)
    # Only enrich top 50 (most recent) to keep API calls reasonable
    to_enrich = arts[:50]
    rest       = arts[50:]
    enriched   = ai_enrich_articles(to_enrich, batch_size=10)
    return _json.dumps(enriched + rest, default=str)

# Build a stable cache key from first 50 titles
...
with st.spinner("🤖 AI analysing news sentiment..."):
    try:
        ...
        all_articles_ai = all_articles  # fallback: use unenriched

# ─────────────────────────────────────────────────────────────
# TABS  (FII/DII tab removed — data unreliable)
# ─────────────────────────────────────────────────────────────

news_cats=list(FEED_SOURCES.keys())
tab_brief,tab_all,tab_prio,tab_port,tab_rbi,tab_nse,tab_sebi,tab_lw,*cat_tabs=st.tabs(
    ["☀️ Morning Briefing","📋 All News","🔴 Priority","📂 Portfolio",
     "🏛️ RBI Circulars","🔵 NSE Circulars","⚖️ SEBI","🌐 Live Wire"]+news_cats
)

with tab_brief:
    render_morning_briefing(all_articles_ai, st.session_state.get("portfolio", load_portfolio()))
