"""
JM Financial | Risk Intelligence Dashboard  v10.0
==================================================
CHANGES FROM v9:
✅ Import = ONE CLICK → st.file_uploader rendered directly as a native button
   (no toggle, no 2-step). Uses label_visibility="collapsed" + Streamlit's
   own "Browse files" button which opens the OS file picker in one click.
✅ Two buttons layout: [📂 Import Portfolio]  [🗑 Delete Portfolio]
   Import button IS the file uploader — clicking it opens file picker directly.
✅ Portfolio news limited to 5 articles AND current month only (no previous month)
✅ Al Jazeera RSS added to Live Wire (Middle East + All News feeds)
✅ IP Login Log — records visitor IP, timestamp, user-agent to login_log.json
   Viewable inside Admin Panel only
✅ All flash suppression CSS from v9 preserved
...
def is_current_month(dt):
    """Returns True only if dt falls in the current calendar month."""
    if dt.tzinfo is None: dt = dt.replace(tzinfo=timezone.utc)
...
# ─────────────────────────────────────────────
# IP LOGIN LOG
# Why: Tracks every dashboard visitor's IP, timestamp, user-agent.
# Stored in login_log.json (max 500 entries, FIFO).
# Visible only inside Admin Panel → Login Logs section.
# ─────────────────────────────────────────────

def record_login():
    """
    Called once per session (guarded by session_state flag).
    Captures IP from Streamlit's request headers via st.context (Streamlit ≥1.31)
    or falls back to the X-Forwarded-For header.
    """
    if st.session_state.get("_login_recorded"):
        return
    st.session_state["_login_recorded"] = True

    # Attempt to get real IP from headers (works on Streamlit Cloud + most proxies)
    ip = "unknown"
    ua = "unknown"
    try:
        # Streamlit ≥1.31 exposes request headers via st.context.headers
    ...
    logs.insert(0, entry)
    logs = logs[:500]   # keep last 500 entries only
...
def fetch_stock_news_gn(nse, name):
    """
    Fetch news for a stock. Returns max 5 articles from CURRENT MONTH ONLY.
    If no current-month news exists, returns empty list (blank is acceptable per user).
    """
    seen, items = set(), []
    now = datetime.now(timezone.utc)
    for q in [f"{nse} NSE stock India", f"{name} stock NSE"]:
        try:
            for e in feedparser.parse(gn(q)).entries[:20]:
                ...
                # STRICT: current month only
                if not is_current_month(dt): continue
                seen.add(t)
...
# Live Wire — Investing.com India + Al Jazeera Middle East + Google News
# Al Jazeera RSS: https://www.aljazeera.com/xml/rss/all.xml (all news)
#                 https://www.aljazeera.com/xml/rss/middleeast.xml (Middle East focused)
LIVEWIRE_FEEDS = [
    ...
    "https://www.aljazeera.com/xml/rss/all.xml",            # Al Jazeera: all breaking news
    "https://www.aljazeera.com/xml/rss/middleeast.xml",     # Al Jazeera: Middle East focus (Iran war)
...
def fetch_livewire():
    ...
    for url in LIVEWIRE_FEEDS:
        try:
            h={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64)","Accept":"application/rss+xml,*/*"}
...
/* ── FLASH ELIMINATION (all known Streamlit rerun visual mechanisms) ── */
...
/* ── FILE UPLOADER STYLED DARK ──
   The file_uploader widget is the actual Import button.
   We style its internal "Browse files" button to look like all other sidebar buttons.
   No hiding — this IS the clickable element. One click → OS file picker opens.
─────────────────────────────────────────────────────────────────────────────── */
...
/* Hide the drag-drop zone area, keep only the Browse button */
...
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] small {
    display: none !important;   /* hide "Drag and drop" / "Limit" text */
}
...
.market-monitor {
    background:#1a237e; border-radius:8px;
    padding:10px 16px 12px; margin-bottom:12px;
}
...
/* ── LOGIN LOG TABLE ── */
.log-table { width:100%;border-collapse:collapse;font-size:9pt !important;margin-top:8px; }
.log-table th { background:#2d3748;color:#90caf9 !important;padding:5px 8px;text-align:left;border-bottom:1px solid #4a5568; }
.log-table td { padding:4px 8px;border-bottom:1px solid #2d3748;color:#c9d1d9 !important;word-break:break-all; }
.log-table tr:hover td { background:#2d3748 !important; }
...
with st.sidebar:
    ...
    if is_admin:
        ...
        # ── LOGIN LOGS ──
        st.markdown("#### 🛡️ Login / Access Log")
        st.markdown(
            "<div style='font-size:9pt;color:#90caf9;margin-bottom:6px;'>"
            "Each row = one browser session. IP captured from request headers.</div>",
            unsafe_allow_html=True
        )
        logs = load_login_logs()
        if not logs:
            st.caption("No login records yet.")
        else:
            # Show last 20 in a dark table
            rows_html = ""
            for log in logs[:20]:
                ts = log.get("timestamp","—")
                ip = log.get("ip","—")
                ua = log.get("user_agent","—")[:60] + ("…" if len(log.get("user_agent","")) > 60 else "")
                rows_html += f"<tr><td>{ts}</td><td>{ip}</td><td style='font-size:8pt !important;color:#90a4ae !important;'>{ua}</td></tr>"
            st.markdown(f"""
            <table class="log-table">
              <thead><tr><th>Timestamp (IST)</th><th>IP Address</th><th>Browser / Device</th></tr></thead>
              <tbody>{rows_html}</tbody>
            </table>""", unsafe_allow_html=True)
            st.markdown(f"<div style='font-size:8.5pt;color:#484f58;margin-top:4px;'>Showing last 20 of {len(logs)} total sessions.</div>",
                        unsafe_allow_html=True)
            if st.button("🗑 Clear Login Log", use_container_width=True, key="clear_log"):
...
def render_portfolio(portfolio):
    ...
    for stock in portfolio:
        ...
        with st.spinner(f"Loading {name}..."):
            arts = fetch_stock_news_gn(nse, name)  # already filtered: current month only, max 5
        if not arts:
            ...
            continue
        p_n=sum(1 for a in arts if a.get("sentiment")=="positive")
        n_n=sum(1 for a in arts if a.get("sentiment")=="negative")
        u_n=len(arts)-p_n-n_n
        st.markdown(f'<div class="sent-bar"><span>Headlines: <b>{len(arts)}</b></span><span>🟢 <span class="pos">{p_n} Positive</span></span><span>🔴 <span class="neg">{n_n} Negative</span></span><span>⚪ <span class="neu">{u_n} Neutral</span></span></div>', unsafe_allow_html=True)
        for art in arts:
...
def render_livewire(articles):
    ...
    for art in articles:
        ip=art.get("priority",False); sent=art.get("sentiment","neutral")
        link = art.get("link","#")
        # Mark Al Jazeera articles with AJ badge
        ...
        ago=time_ago(dt_o)
        pb='<span class="bp">⚡ PRIORITY</span>' if ip else ""
        src_badge='<span class="baj">📡 AL JAZEERA</span>' if is_aj else '<span class="blw">🌐 LIVE</span>'
        sb=('<span class="bpo">▲ POSITIVE</span>' if sent=="positive" else '<span class="bne">▼ NEGATIVE</span>' if sent=="negative" else "")
        st.markdown(f'<div class="{cls}"><a class="card-title" href="{link}" target="_blank">{art["title"]}</a><div class="card-meta"><span>{ago}</span>{src_badge}{sb}{pb}</div></div>', unsafe_allow_html=True)
