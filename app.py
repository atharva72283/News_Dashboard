"""
JM Financial | Risk Intelligence Dashboard  v19.0
==================================================
KEY CHANGES:
✅ "None" UI leak fixed (strict variable handling).
✅ PDF completely redesigned for individual stock AI analysis.
✅ User's exact Senior Risk Analyst prompt integrated for PDF generation.
✅ Dynamic progress updater added during PDF generation.
"""
...
def generate_briefing_pdf(portfolio, market_data, progress_bar=None, status_text=None):
    ...
    top_exposures = sorted(portfolio, key=lambda x: x.get("position_crs",0), reverse=True)
    ...
    for idx, stock in enumerate(top_exposures):
        ...
        if progress_bar and status_text:
            ...
            status_text.text(f"Analysing {nm} ({idx + 1}/{total_stocks})...")
...
with st.sidebar:
    ...
    if is_admin:
        ...
        if st.button("⬇ Generate AI Risk Report (PDF)", use_container_width=True):
            ...
            pdf_progress.empty() # Clear progress bar when done
            pdf_status.empty()   # Clear status text when done
