"""
JM Financial | Risk Intelligence Dashboard  v16.0
==================================================
KEY CHANGES:
✅ Regex cleaner added to strip publisher names (e.g., "- NDTV Profit") before analysis
✅ Mistral AI prompt hardened with strict Risk Analyst instructions
✅ AI enrichment routing fixed: AI tags now appear on Live Wire, Categories, and Portfolio tabs
✅ AI processing limit increased to cover more ground across tabs
...
def clean_headline(title):
    """Strips publication names from the end of headlines (e.g., ' - NDTV Profit')"""
    ...
    for i in range(0, len(needs_ai), batch_size):
        ...
        for j, art in enumerate(batch):
            ...
            if j in sentiment_map:
                ...
                new_art["sentiment"]   = get_sentiment(new_art["title"]) # Fallback to cleaned keyword logic
...
def get_ai_enriched_articles(articles_key: str, articles_json: str, limit: int = 80) -> str:
    ...
    to_enrich = arts[:limit] # Increased limit to ensure Live Wire and categories get hit
    rest       = arts[limit:]
...
with tab_lw:
    lw_ai = [a for a in all_articles_ai if a.get("category") == "🌐 Live Wire"]
    render_livewire(lw_ai if lw_ai else livewire_articles)

for tab,cat in zip(cat_tabs,news_cats):
    with tab:
        st.markdown(f'<div class="cat-header">{cat}</div>',unsafe_allow_html=True)
        cat_ai = [a for a in all_articles_ai if a.get("category") == cat]
        render_news_cards(cat_ai if cat_ai else all_data.get(cat, []))
