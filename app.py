...
def fetch_stock_quote(raw):
    # Uses yf.download() — high rate limit endpoint.
    # yf.Ticker().info was causing Too Many Requests — it hits a separate
    # Yahoo endpoint that is aggressively rate-limited.
    # TTL=600s means same ticker wont re-call Yahoo for 10 minutes.
    inp = raw.strip().upper()
    if len(inp)==12 and inp.startswith("IN") and inp[2:].isalnum():
        nse = ISIN_TO_NSE.get(inp)
        sym = (nse+".NS") if nse else None
        if not sym: return {"found":False,"error":f"ISIN {inp} not in local map. Try NSE code.","symbol":inp}
    elif inp.isdigit():
    ...
    def _fetch(s):
        try:
            ...
            if df is None or df.empty:
                return {"found":False,"error":"No data returned","symbol":s}
            close = df["Close"].dropna()
            if len(close) < 2:
                return {"found":False,"error":"Insufficient history","symbol":s}
            price = float(close.iloc[-1])
            prev  = float(close.iloc[-2])
            if price <= 0 or prev <= 0:
                return {"found":False,"error":"Invalid price","symbol":s}
            c   = price - prev
            pct = (c / prev) * 100
            # Name from fast_info only — NO .info call, no rate limit hit
            try:
                nm = getattr(yf.Ticker(s).fast_info, "name", None) or s
            except Exception:
                nm = s
            return {"found":True,"name":nm,"symbol":s,
                    "price":price,"change":c,"pct":pct,"error":None}
        except Exception as e:
            return {"found":False,"error":str(e),"symbol":s}

    res = _fetch(sym)
    if not res["found"] and sym.endswith(".NS"):
        res = _fetch(sym.replace(".NS",".BO"))
    if not res["found"]:
        res = _fetch(inp)
    return res
...
def fetch_stock_news_gn(nse, name):
    """Max 5 articles. Prefers current month; falls back to last 30 days if month is sparse."""
    seen, items = set(), []
    for q in [f"{nse} NSE stock India", f"{name} stock NSE"]:
        try:
            for e in feedparser.parse(gn(q)).entries[:20]:
                ...
                # Accept current month OR last 30 days (handles early-month sparse periods)
                if not is_recent(dt, days=30): continue
                seen.add(t)
...
def fetch_market_data():
    """
    Robust intraday ticker fetch.
    Strategy:
    1. Download last 5 days of 1-minute data for each symbol.
       The most recent bar is the true last-traded price (intraday or most recent close).
    2. Previous close = last bar of the previous session.
    3. Falls back to fast_info if download fails.
    TTL=60s so tickers update every minute during market hours.
    """
    res = {}
    for name, (sym, unit, _) in MARKET_TICKERS.items():
        price, prev, fetched_ok = None, None, False
        # Primary: 1-minute intraday data — gives real live price during session
        try:
            ...
            if df is not None and not df.empty:
                ...
                if close_col in df.columns:
                    ...
                    if len(series) >= 2:
                        ...
                        # Previous close = last bar of the previous calendar day
                        today_str = datetime.now(IST).strftime("%Y-%m-%d")
                        prev_bars = series[series.index.strftime("%Y-%m-%d") < today_str]
            ...
            pass
        # Fallback: fast_info
        if not fetched_ok or price is None:
            try:
                ...
                pass
        if fetched_ok and price and prev and prev > 0:
            c   = price - prev
            pct = (c / prev) * 100
...
def fetch_fii_flow():
    ...
    nseindia.com is a JavaScript-rendered React SPA. When fetched with requests/
    BeautifulSoup, the HTML returned has empty container divs — the actual table
    data is populated later by JavaScript in the browser. BeautifulSoup never sees
    the table rows because they do not exist in the raw HTML response.

    THE FIX — USE NSE's INTERNAL JSON API:
    The NSE website itself fetches FII/DII data from an internal REST endpoint:
      https://www.nseindia.com/api/fiidiiTradeReact
    This endpoint returns a clean JSON array with Date, FII_NET_TRADE, DII_NET_TRADE.
    No JavaScript rendering needed. Direct JSON. Much more reliable.

    The trick is sending the right headers — NSE validates Referer and User-Agent.
    We also hit the homepage first to get a session cookie, then call the API.

    Falls back to 3 Google News queries if the API fails.
    """
    import re as _re
    result = {"fii_net": None, "dii_net": None, "headline": None,
              "dt": None, "data_date": None, "source": None}

    # ── PRIMARY: NSE internal JSON API ────────────────────────────────────────
    try:
        ...
        # Step 2: Call the FII/DII JSON API
        api_headers = {
            **base_headers,
        ...
        resp = session.get(
            "https://www.nseindia.com/api/fiidiiTradeReact",
            headers=api_headers, timeout=12, verify=False,
            proxies=PROXIES or {}
        )
        data = resp.json()

        # API returns list of dicts — most recent date first
        # Keys (approximate): date/Date, fiiNetTrade/FII_NET_TRADE, diiNetTrade/DII_NET_TRADE
        if isinstance(data, list) and len(data) > 0:
            row = data[0]   # most recent entry
            # Try multiple key name variants NSE has used across versions
            date_val = (row.get("date") or row.get("Date") or
                        row.get("TRADE_DATE") or row.get("tradeDate") or "")
            fii_val  = (row.get("fiiNetTrade") or row.get("FII_NET_TRADE") or
                        row.get("fii_net_trade") or row.get("netValue") or
                        row.get("FII") or 0)
            dii_val  = (row.get("diiNetTrade") or row.get("DII_NET_TRADE") or
                        row.get("dii_net_trade") or row.get("DII") or 0)
            # Some variants nest under "cashMarket" key
            if "cashMarket" in row:
                cm = row["cashMarket"]
                fii_val = cm.get("fiiNetTrade") or cm.get("FII_NET_TRADE") or fii_val
                dii_val = cm.get("diiNetTrade") or cm.get("DII_NET_TRADE") or dii_val

            try: fii_f = float(str(fii_val).replace(",","").replace(" ",""))
            except: fii_f = None
            try: dii_f = float(str(dii_val).replace(",","").replace(" ",""))
            except: dii_f = None

            if fii_f is not None:
                result["fii_net"]   = fii_f
                result["dii_net"]   = dii_f
                result["data_date"] = str(date_val)[:12]
                result["dt"]        = datetime.now(timezone.utc)
                result["source"]    = "NSE API"
                fp = "+" if fii_f >= 0 else ""
                dp = "+" if (dii_f or 0) >= 0 else ""
                result["headline"]  = (f"FII: {fp}{fii_f:,.2f} Cr  |  "
                                       f"DII: {dp}{(dii_f or 0):,.2f} Cr"
                                       f"  (as of {str(date_val)[:12]})")
                return result
    except Exception:
        pass

    # ── FALLBACK 1: ET Markets FII data URL ───────────────────────────────────
    try:
        et_url = "https://economictimes.indiatimes.com/markets/stocks/news/fii-dii-data"
        h = {"User-Agent": "Mozilla/5.0", "Referer": "https://economictimes.indiatimes.com/"}
        resp = requests.get(et_url, headers=h, timeout=10, verify=False, proxies=PROXIES or {})
        from bs4 import BeautifulSoup
        soup = BeautifulSoup(resp.content, "lxml")
        # ET renders FII/DII in a static table unlike NSE
        ...
        for tbl in tables:
            ...
            for row in rows:
                ...
                date_m = _re.search(r"(\d{1,2}[\-/]\w{3}[\-/]\d{2,4}|\d{1,2}[\-/]\d{1,2}[\-/]\d{2,4})", cells[0])
                if date_m:
                    nums = [_re.sub(r"[^\d\.\-\+]","",c) for c in cells[1:]]
                    valid = []
                    for n in nums:
                        try: valid.append(float(n))
                        except: pass
                    if len(valid) >= 2:
                        result.update({"fii_net":valid[0],"dii_net":valid[1],
                                       "data_date":date_m.group(1),"source":"ET Markets",
                                       "dt":datetime.now(timezone.utc)})
                        fp = "+" if valid[0]>=0 else ""
                        dp = "+" if valid[1]>=0 else ""
                        result["headline"] = f"FII: {fp}{valid[0]:,.2f} Cr | DII: {dp}{valid[1]:,.2f} Cr (ET Markets)"
                        return result
    except Exception:
        pass

    # ── FALLBACK 2: Google News — headline text only ───────────────────────────
    try:
        for q in ["FII DII net NSE India provisional crore today",
                  "FII buying selling India today crore NSE",
                  "foreign institutional investor net India today"]:
            ...
            for e in feed.entries[:10]:
                ...
                if ("fii" in tl or "fpi" in tl) and "dii" in tl and is_recent(dt, days=1):
                    ...
                    result["data_date"]  = to_ist(dt).strftime("%d %b %Y")
                    ...
                    if len(valid) >= 2:
                        result["fii_net"] = valid[0]
                        result["dii_net"] = valid[1]
                    return result
    except Exception:
        pass
...
def fetch_rbi_circulars():
    ...
    # RBI Direct (ASPX) scraping removed — RSS + Google News only
...
def _safe(text: str, limit: int = 200) -> str:
    """Strip non-latin-1 chars so fpdf2 never crashes on Unicode arrows/emojis."""
...
def generate_briefing_pdf(all_articles, rbi_items, nse_items, sebi_items,
    ...
    """
    Morning risk briefing PDF. All text sanitised to Latin-1 before writing
    so Unicode arrows (▲▼), rupee signs, emojis never crash the encoder.
    pdf.output() return value handled for both fpdf and fpdf2 versions.
    """
    try:
    ...
    pdf.set_xy(10, 7)
    pdf.cell(130, 8, "JM FINANCIAL  |  RISK INTELLIGENCE BRIEFING")
    pdf.set_font("Helvetica", "", 8)
    pdf.set_xy(140, 9)
    pdf.cell(60, 6, _safe(now_ist), align="R")
    ...
    def section(title, fr, fg, fb):
        ...
        pdf.cell(190, 7, "  " + title, fill=True)
        pdf.ln(8)
    ...
    for name, data in market_data.items():
        p   = data.get("price"); c = data.get("change") or 0; pct = data.get("pct") or 0
        unit = data.get("unit", "")
        if p is None: continue
        arrow = "+" if c >= 0 else "-"
        ps = f"{unit}{p:,.0f}" if name in ("NIFTY 50","SENSEX","BANK NIFTY") else f"{unit}{p:,.2f}"
        line = _safe(f"{name}: {ps}  ({arrow}{abs(pct):.2f}%)")
    ...
    prio = [a for a in all_articles
            if a.get("priority") and is_recent(a.get("dt", datetime.now(timezone.utc)), days=1)][:10]
    if prio:
        for a in prio:
            pdf.set_text_color(160, 0, 0); pdf.cell(6, 5, ">")
            pdf.set_text_color(0, 0, 0)
            pdf.multi_cell(184, 5, _safe(a.get("title", ""), 120))
    else:
        pdf.set_text_color(130, 130, 130)
        pdf.cell(190, 5, "No priority alerts in the last 24 hours."); pdf.ln(6)
    pdf.set_text_color(0, 0, 0); pdf.ln(3)

    # New Circulars
    section("NEW CIRCULARS  (RBI / NSE / SEBI  -  last 24 hours)", 255, 252, 220)
    all_circs = ([(a,"RBI") for a in rbi_items] +
                 [(a,"NSE") for a in nse_items] +
                 [(a,"SEBI") for a in sebi_items])
    recent_circs = [(a,s) for a,s in all_circs
                    if is_recent(a.get("dt", datetime.now(timezone.utc)), days=1)][:12]
    if recent_circs:
        for a, src in recent_circs:
            pdf.set_font("Helvetica","B",9); pdf.set_text_color(180, 60, 0)
            pdf.cell(14, 5, f"[{src}]")
            pdf.set_font("Helvetica","",9); pdf.set_text_color(0, 0, 0)
            pdf.multi_cell(176, 5, _safe(a.get("title",""), 110))
    else:
        pdf.set_text_color(130, 130, 130)
        pdf.cell(190, 5, "No new circulars in the last 24 hours."); pdf.ln(6)
    pdf.set_text_color(0, 0, 0); pdf.ln(3)

    # Portfolio Summary — sorted by position value (highest first)
    section("PORTFOLIO RISK SUMMARY", 230, 245, 230)
    total_pos = sum(p.get("position_crs", 0) for p in portfolio)
    pdf.set_font("Helvetica","B",9)
    pdf.cell(190, 5, f"Total Exposure: INR {total_pos:,.1f} Cr  |  Stocks: {len(portfolio)}"); pdf.ln(7)
    pdf.set_font("Helvetica","",9)
    port_sorted = sorted(portfolio, key=lambda x: x.get("position_crs",0), reverse=True)
    for stock in port_sorted[:20]:
        nm  = _safe(stock.get("name",""), 30)
        nse = _safe(stock.get("nse_code",""))
        pos = stock.get("position_crs", 0)
        pct_p = (pos / total_pos * 100) if total_pos > 0 else 0
        pdf.cell(65, 5, nm)
        pdf.cell(25, 5, nse)
        pdf.cell(40, 5, f"INR {pos:,.1f} Cr")
        pdf.set_text_color(100, 100, 100)
        pdf.cell(40, 5, f"({pct_p:.1f}% of book)"); pdf.ln(5)
        pdf.set_text_color(0, 0, 0)
    ...
    # Handle both fpdf (returns string) and fpdf2 (returns bytearray)
    raw = pdf.output()
...
/* File uploader shown normally when toggled — styled dark below */
...
# Merge Live Wire into All News — this is what was missing
...
# ── FII / DII Flow Bar ──
fii_html = '<div class="fii-bar"><span>📊 FII/DII Provisional Data'
data_date = fii_data.get("data_date")
fii_src   = fii_data.get("source","")
if data_date:
    fii_html += f' &nbsp;·&nbsp; <span style="color:#ffd54f !important;font-weight:600;">As of {data_date}</span>'
if fii_src:
    fii_html += f' &nbsp;·&nbsp; <span style="color:#90caf9 !important;font-size:8.5pt !important;">{fii_src}</span>'
fii_html += ':</span>'
...
if fn is not None:
    ...
    fp = "+" if fn >= 0 else ""
    dp = "+" if (dn or 0) >= 0 else ""
    fii_html += f'<span>FII: <span class="{fc}">{fp}&#8377;{fn:,.0f} Cr</span></span>'
    if dn is not None:
        fii_html += f'<span>DII: <span class="{dc}">{dp}&#8377;{dn:,.0f} Cr</span></span>'
    if fn < 0 and (dn or 0) < 0:
        fii_html += '<span style="color:#ff8a80 !important;font-weight:700;font-size:9.5pt !important;">⚠ Both Selling</span>'
    elif fn >= 0 and (dn or 0) >= 0:
        fii_html += '<span style="color:#80e27e !important;font-weight:700;font-size:9.5pt !important;">✓ Both Buying</span>'
    hl = fii_data.get("headline","")[:90]
    if hl:
        fii_html += f'<span class="fii-neu" style="font-size:8.5pt !important;">{hl}</span>'
else:
    fii_html += '<span class="fii-neu">Provisional data available after 5 PM IST &nbsp;|&nbsp; <a href="https://www.nseindia.com/reports/fii-dii" target="_blank" style="color:#90caf9 !important;">NSE FII-DII Page</a></span>'
fii_html += '</div>'
...
def render_news_cards(articles, apply_dedup=True):
    # apply_dedup=True  → global dedup (only used for tab_all)
    # apply_dedup=False → local dedup only (used for all category tabs)
    if apply_dedup:
        items = dedup_articles(articles)
    else:
        # Local dedup: remove duplicates within this list only, never touch GLOBAL_SEEN
        seen_local, items = set(), []
        for a in articles:
            t = a.get("title","")
...
def render_portfolio(portfolio):
    ...
    # Sort by portfolio position value HIGHEST FIRST (preserves Excel import order by position)
    # Risk score badge shown per stock but does NOT reorder
    portfolio_sorted = sorted(portfolio, key=lambda x: x.get("position_crs", 0), reverse=True)
    portfolio_scored = []
    for stock in portfolio_sorted:
...
with tab_all:
    # All News = FEED_SOURCES news + manual items + live wire merged, global dedup
    render_news_cards(all_articles, apply_dedup=True)
...
with tab_sebi:
    st.markdown("<div style='font-size:10pt;color:#6a0dad;font-weight:600;margin-bottom:8px;'>⚖️ SEBI Circulars, Notifications & Enforcement Orders &nbsp;·&nbsp; sebi.gov.in RSS + Google News</div>",unsafe_allow_html=True)
...
# Category tabs: apply_dedup=False so they show their OWN articles
# independently — not filtered by what tab_all already showed
for tab,cat in zip(cat_tabs,news_cats):
    with tab:
        st.markdown(f'<div class="cat-header">{cat}</div>',unsafe_allow_html=True)
        cat_arts = [{**a,"category":cat,"manual":False} for a in all_data.get(cat,[])]
        render_news_cards(cat_arts, apply_dedup=False)
