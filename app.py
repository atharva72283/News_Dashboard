"""
JM Financial | Risk Intelligence Dashboard  v20.0
==================================================
KEY CHANGES:
✅ Streamlit "Magic" bug fixed — suppressed 'None' UI leaks during PDF generation.
✅ PDF AI Analysis capped strictly at Top 20 portfolio exposures.
"""
...
# ─────────────────────────────────────────────────────────────
# PDF EXPORT (Individual Stock Analysis)
# ─────────────────────────────────────────────────────────────
...
def generate_briefing_pdf(portfolio, market_data, progress_bar=None, status_text=None):
    try:
    ...
    pdf.set_xy(10,7)
    pdf.cell(130,8,"JM FINANCIAL  |  INDIVIDUAL PORTFOLIO RISK BRIEFING")
    ...
    # PART 2: INDIVIDUAL PORTFOLIO RISK (AI Analyzed)
    section_header("INDIVIDUAL PORTFOLIO RISK ANALYSIS", 230,245,230)
    
    # Restrict strictly to top 20 portfolio exposures
    top_exposures = sorted(portfolio, key=lambda x: x.get("position_crs",0), reverse=True)[:20]
    total_stocks = len(top_exposures)
    
    for idx, stock in enumerate(top_exposures):
        nm = stock.get("name","")
        nse = stock.get("nse_code","")
        pos = stock.get("position_crs",0)
        
        if progress_bar and status_text:
            # Assigning to _ prevents Streamlit from leaking 'None' to UI
            _ = progress_bar.progress((idx + 1) / total_stocks)
            _ = status_text.text(f"Analysing {nm} ({idx + 1}/{total_stocks})...")
        
        # 1. Fetch News
        arts = fetch_stock_news_gn(nse, nm)
        
        # 2. AI Analysis
        ai_summary = ""
        if arts:
            headlines = " | ".join([clean_headline(a["title"]) for a in arts[:4]])
            ai_prompt = (
                f"As a Senior Risk Management Analyst at JM Financials, analyse the following news:\n{headlines}\n\n"
                f"and give me an overall latest risk remark on the company. Consider the latest Financials, liquidity in market, "
                f"promoter news, fundamentals and technical analysis and the history of company to give me a 3-4 liner risk review of company."
            )
            
            raw_summary = _mistral_call([{"role": "user", "content": ai_prompt}], max_tokens=150)
            if raw_summary:
                ai_summary = raw_summary
            else:
                ai_summary = "AI analysis currently unavailable due to API limits. No immediate critical flags detected in recent feeds."
        else:
            ai_summary = "No recent news available to assess current risk for this company."
            
        # 3. Print to PDF
        pdf.set_font("Helvetica", "B", 10)
        pdf.set_text_color(26,35,126) # JM Blue
        pdf.cell(190, 6, _clean_for_pdf(f"{nm} ({nse}) — Exposure: Rs. {pos:,.1f} Cr"))
        pdf.ln(6)
        
        pdf.set_font("Helvetica", "", 9)
        pdf.set_text_color(0,0,0)
        pdf.multi_cell(190, 5, _clean_for_pdf(ai_summary, limit=1500))
        pdf.ln(4)
...
with st.sidebar:
    ...
    if is_admin:
        ...
        # UI Placeholder for PDF generation progress
        pdf_progress = st.progress(0.0)
        pdf_status = st.empty()
        
        if st.button("⬇ Generate AI Risk Report (PDF)", use_container_width=True):
            with st.spinner("Compiling Individual Stock Analysis..."):
                pdf_bytes = generate_briefing_pdf(
                    st.session_state.get("portfolio", load_portfolio()), 
                    market_data,
                    progress_bar=pdf_progress,
                    status_text=pdf_status
                )
            
            # Swallowing None to prevent Streamlit UI leaks
            _ = pdf_progress.empty()
            _ = pdf_status.empty()
...
def render_portfolio(portfolio):
    ...
    for stock in sorted(portfolio, key=lambda x: x.get("position_crs",0), reverse=True):
        ...
        # Fetch and explicitly run AI over portfolio items
        ...
        if raw_arts:
            ...
            try:
                _ai_port_res = get_ai_enriched_articles(_port_key, _port_json, limit=5) # Run AI on the top 5
