...
def fetch_fii_flow():
    """
    Scrape FII/DII provisional data from NSE India official page:
    https://www.nseindia.com/reports/fii-dii
    The page publishes provisional FII/DII cash market figures daily after 5 PM IST.
    Returns: fii_net (Cr), dii_net (Cr), data_date (string), source label.
    Falls back to Google News if NSE scraping fails.
    """
    import re as _re
    result = {"fii_net": None, "dii_net": None, "headline": None,
              "dt": None, "data_date": None, "source": None}

    # Primary: NSE FII-DII reports page
    try:
        ...
        resp = requests.get("https://www.nseindia.com/reports/fii-dii",
                            headers=headers, timeout=15, verify=False,
                            proxies=PROXIES or {})
        soup = BeautifulSoup(resp.content, "lxml")

        # NSE table: columns are Date, FII Net, DII Net (approx)
        # Look for the most recent row
        rows = soup.find_all("tr")
        for row in rows[:20]:
            cells = [td.get_text(strip=True) for td in row.find_all(["td","th"])]
            if not cells or len(cells) < 3: continue
            # Skip header rows
            if any(h in " ".join(cells).lower() for h in ["date","fii","dii","category"]):
                # Try to extract numeric values from this or the next data row
                continue
            # Look for date-like pattern in first cell
            date_match = _re.search(r'(\d{1,2}[\-/]\d{1,2}[\-/]\d{2,4})', cells[0])
            if date_match:
                ...
                if len(valid_nums) >= 2:
                    result["fii_net"]   = valid_nums[0]
                    result["dii_net"]   = valid_nums[1]
                    result["data_date"] = date_match.group(1)
                    result["dt"]        = datetime.now(timezone.utc)
                    result["source"]    = "NSE India"
                    result["headline"]  = (f"NSE FII/DII Data as of {date_match.group(1)}: "
                                           f"FII {('+' if valid_nums[0]>=0 else '')+str(round(valid_nums[0],2))} Cr  |  "
                                           f"DII {('+' if valid_nums[1]>=0 else '')+str(round(valid_nums[1],2))} Cr")
                    return result
    except Exception:
        pass

    # Fallback: Google News headline (no number extraction — display text only)
    try:
        q = "FII DII net buy sell NSE India crore today provisional"
        feed = feedparser.parse(gn(q))
        for e in feed.entries[:8]:
            t  = e.get("title","").strip()
            dt = parse_dt(e)
            tl = t.lower()
            if ("fii" in tl or "fpi" in tl or "dii" in tl) and is_recent(dt, days=1):
                result["headline"] = t
                result["dt"]       = dt
                result["source"]   = "Google News"
                result["data_date"] = to_ist(dt).strftime("%d %b %Y")
                nums = _re.findall(r'[\-\+]?\s*[\d,]+(?:\.\d+)?', t)
                if len(nums) >= 2:
                    try:
